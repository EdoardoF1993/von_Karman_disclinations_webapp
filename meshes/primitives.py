...
def mesh_rectangle_gmshapi(
    ...
    # Create rectangle
    domain = gmsh.model.occ.addRectangle(-L / 2, -H / 2, 0.0, L, H, tag=PHYSICAL_BULK_TAG)
    gmsh.model.geo.synchronize()

    # Tag boundary and surface
    boundary_lines = gmsh.model.getEntities(dim=1)
    gmsh.model.addPhysicalGroup(tdim - 1, [b[1] for b in boundary_lines], tag=PHYSICAL_BND_TAG)
    gmsh.model.addPhysicalGroup(tdim, [domain], tag=PHYSICAL_BULK_TAG)

    # Set element order BEFORE mesh generation
